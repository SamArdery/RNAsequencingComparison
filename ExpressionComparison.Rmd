...
data <- load(here("DataCollection", "DataCollection.RData"))
...
Here we pivot the rows of each of the pearson data tables to prepare them to be combined so they can be plotted on the same graph. 
```{r}
PearsonGenes %>%
  mutate(Type = "Genes") %>%
...
Here we combine the two and plot them together. 
```{r}
...
We can filter out all three major biotype categories into their own data tables
```{r}
pseudos <- filter(Allbiotypes, str_detect(Allbiotypes$Biotype, "pseudogene"))
prots <- filter(Allbiotypes, str_detect(Allbiotypes$Biotype, "protein")) 
rnas <- filter(Allbiotypes, str_detect(Allbiotypes$Biotype, "protein") == FALSE & str_detect(Allbiotypes$Biotype, "pseudogene") == FALSE)
```

With the filtered data tables, we can determine the sum of the counts for each of the major three biotype categories that we have determined.
```{r}
pseudos %>%
  add_row(Biotype = "Pseudogenes", Matched = sum(pseudos$Matched), Paired = sum(pseudos$Paired), Single = sum(pseudos$Single)) -> pseudos
prots %>%
  add_row(Biotype = "Protein Coding", Matched = sum(prots$Matched), Paired = sum(prots$Paired), Single = sum(prots$Single)) -> prots
rnas %>%
  add_row(Biotype = "Small RNAs", Matched = sum(rnas$Matched), Paired = sum(rnas$Paired), Single = sum(rnas$Single)) -> rnas
```

These three categories can then be joined again and the quantities of each biotype can be displayed in a table.
```{r}
BiotypeCats <- full_join(rnas[18,], pseudos[2,], by = c("Biotype", "Matched", "Paired", "Single"))
BiotypeCats <- full_join(BiotypeCats, prots[2,], by = c("Biotype", "Matched", "Paired", "Single")) 
BiotypeCats
```

The tables show that SE analysis uniquely maps to a large quantity of pseudogenes. Hypergeomentric testing using an online calculator (<https://systems.crump.ucla.edu/hypergeometric/>). The matched data was used as the universe, with the protein-coding or pseudogene quantities within that data for the universe successes. The unique PE or unique SE data were the sample and the respective quantities of protein coding genes or pseudogenes were the sample successes. It showed that pseudogenes are over-represented 1.34 fold (p = 1.34 E -55), which is why the SE pseudogenes are highlighted in red on the table. In a similar fashion, hypergeometric testing was performed on the protein coding genes in PE analysis and they were found to be over-represented 2.7 fold (p = 2.74 E -130), showing that paired-end analysis uniquely maps more protein coding genes than would be expected should the sequencing methods be equal.

This can also be shown using a Venn diagram.
```{r}
...
plot(euler(Venn), quantities = list(TRUE, cex = 2), fill = c("coral2", "cyan3", "lavenderblush"), alpha = 0.5, main = "Gene Overlap in SE and PE Data", labels = list(col = "black", font = 2, cex = 2), )
```
...
As done previously, we separate the median values into the three major biotype categories present in the sample and we can then plot based on these categories. 
```{r}
for(i in 1:nrow(Medians)){
  if(str_detect(Medians[i,2], "rotein") == TRUE) {
    Medians[i,2] <- "Protein Coding"
  } else if (str_detect(Medians[i,2], "seudogene")) {
    Medians[i,2] <- "Pseudogene"
  } else {
    Medians[i,2] <- "Small RNA"
  }
}

Medians %>%
  pivot_longer(cols = -c(GeneID, Biotype), names_to = "SeqMethod", values_to = "ExpressionMedian") %>%
  ggplot(aes(x = log1p(ExpressionMedian), y = Biotype, fill = SeqMethod)) +
    geom_violin() +
...
## **Writing Data for Other Analysis**
...
write.csv(uniquePE, 'UniquePEexp.csv')
#writing the unique single-end data to a csv for use in eQTL analysis
write.csv(uniqueSE, 'UniqueSEexp.csv')
#writing the PearsonGenes data frame for STAR analysis
save(PearsonGenes, file = "PearsonGenes.RData")
```
