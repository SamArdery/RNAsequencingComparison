...
load("/projects/munger-lab/ArderyProject/src/RNAseq_comp/MappingScripts/STAR_SEMappingData.RData")
load("/projects/munger-lab/ArderyProject/src/RNAseq_comp/Mapping/ScriptsDO_mESC_eQTL_peaks.RData")
rm(peaks, cmd, DO185_mESC_emase_m4.cmd)
n.cores <- 16
final <- STARfinal
STARSEDatacmd <- "This expression data was obtained by running the DataCollection.Rmd file"
...
message("got exprCombat")
covar <- covar[,"sex", drop=FALSE]

message("got past combat")
...
save(map_dat2, pmap, gmap, exprZ, kinship_loco, exprComBat, genoprobs, covar, file = "/projects/munger-lab/ArderyProject/src/RNAseq_comp/MappingScripts/STAR_SEforMapping.RData")
...
cmd <- paste0("This mapping data was created using STAR_SEMappingScript.R on ", Sys.Date())

outfile <- paste0(outdir, "/DO_mESC_starse_eQTL_peaks.RData")
save(peaks, cmd, map_dat, gmap, STARSEDatacmd, file=outfile)
